...
export const en = {
  ...
  jobDetailPage: {
    ...
    listingStatusUnknown: "Not checked",
    checkListing: "Check listing",
    checkingListing: "Checking…",
    checkListingTitle: "Check if this listing is still online",
    rawText: "Job text",
    notes: "Notes",
    createdAt: "Created",
  ...
  mailScanSettings: {
    heading: "Mail scan",
    hint: "Read job-alert emails from local mail folders. Nothing is sent anywhere except the scoring call you configure below.",

    sourcesHeading: "Mail folders",
    sourcesHint:
      "Point at Thunderbird’s mail file, not the .msf next to it. In Dolphin, Local Folders shows Indeed (the mails) and Indeed.msf (search index only — ignored). Use Browse, or Find Thunderbird folders.",
    sourceAdd: "Add folder",
    sourceRemove: "Remove",
    sourceLabel: "Label",
    sourceLabelPlaceholder: "e.g. Indeed alerts",
    sourcePath: "Path",
    sourcePathPlaceholder: "~/.thunderbird/…/Mail/Local Folders/Indeed",
    sourceBrowseMbox: "Browse mail file…",
    sourceBrowseMaildir: "Browse maildir…",
    sourceResolved: (path: string) => `Resolved: ${path}`,
    sourceRedirected:
      "This path was adjusted (symlink or .msf → mail file) — check the resolved path before scanning.",
    sourceTest: "Test read",
    sourceTestResult: (count: number) => `${count} message${count === 1 ? "" : "s"}`,
    sourceTestRange: (from: string, to: string) => `${from} → ${to}`,
    sourceNone: "No folders yet.",
    thunderbirdDetect: "Find Thunderbird folders",
    thunderbirdFound: (n: number) =>
      `Found ${n} Thunderbird mail folder${n === 1 ? "" : "s"} (mbox files under Local Folders)`,
    thunderbirdNone:
    ...
    rescoreBacklogHint: (n: number) =>
      `${n} listing${n === 1 ? "" : "s"} below the cutoff would be scored again. This costs model calls.`,
    rescoreBacklogNone: "Nothing below the cutoff to re-score.",
    deleteAll: "Delete all mail scan data",
    deleteAllHint: "Removes the inbox, dismissals, scores, cursors, and run history. Your jobs are not touched.",
    deleteAllConfirmLabel: "Type DELETE to confirm",
    deleteAllConfirm: "Delete everything",
    deleteAllDone: "Mail scan data deleted.",

    sidecarHeading: "Scanner",
    sidecarMode: (description: string) => `Running as: ${description}`,
    sidecarPinned: "Signature verified",
    sidecarUnpinned: "Development build — signature not checked",
    sidecarMissing: "The mail scanner is not available in this build.",
